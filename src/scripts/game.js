...
class playGame extends Phaser.Scene {
  ...
  create() {
    ...
    this.pattern = this.add.tileSprite(
      game.config.width / 2,
      gameOptions.platformHeight / 2,
      game.config.width,
      gameOptions.platformHeight * 2,
      'pattern'
    );
    ...
    this.cameras.main.startFollow(
      ...
      0.5,
      0,
      -(game.config.height / 2 - game.config.height * gameOptions.firstPlatformPosition)
    );
  ...
  addPlatform(isFirstPlatform) {
    ...
    let platform = this.add.renderTexture(
      game.config.width / 2,
      isFirstPlatform ? game.config.width * gameOptions.firstPlatformPosition : 0,
      game.config.width / 8,
    ...
    platform.assignedVelocityX = isFirstPlatform
      ? 0
      : this.randomValue(gameOptions.platformHorizontalSpeedRange) * Phaser.Math.RND.sign();
  }
  ...
  destroyPlatform() {
    if (this.canDestroy) {
      ...
      closestPlatform.assignedVelocityX =
        this.randomValue(gameOptions.platformHorizontalSpeedRange) * Phaser.Math.RND.sign();
  ...
  positionPlatform(platform) {
    platform.y =
      this.getLowestPlatform() + this.randomValue(gameOptions.platformVerticalDistanceRange);
  ...
  update() {
    ...
    this.platformGroup.getChildren().forEach(function (platform) {
      ...
      let distance =
        (Math.max(0.2, 1 - Math.abs(game.config.width / 2 - platform.x) / (game.config.width / 2)) *
          Math.PI) /
        2;
      platform.body.setVelocityX(platform.assignedVelocityX * distance);
      if (
        (platform.body.velocity.x < 0 && platform.getBounds().left < this.hero.displayWidth / 2) ||
        (platform.body.velocity.x > 0 &&
          platform.getBounds().right > game.config.width - this.hero.displayWidth / 2)
      ) {
